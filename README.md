## DSH 桌面版 v0.3.7

DeepSeek Harness 桌面客户端 v0.3.7

这是由**DSH通过命令自我封装与添加功能**的桌面客户端

<img width="959" height="600" alt="11" src="https://github.com/user-attachments/assets/6283ac0a-ba04-4183-9eb0-18c55f4d2614" />
<img width="958" height="598" alt="22" src="https://github.com/user-attachments/assets/580a5609-70bb-420b-b77e-2f865bd1e593" />
<img width="958" height="599" alt="33" src="https://github.com/user-attachments/assets/73e39cab-427b-4d29-8731-6b88b9df87b5" />
<img width="958" height="596" alt="44" src="https://github.com/user-attachments/assets/98220fd7-28d3-4acc-afc5-50422cc4cf00" />



- 
**下载前的提示**
可以将安装文件放置在非C盘目录下，建议路径不要有中文。

### 下载

- `DSH-Desktop-v0.3.7.zip`：压缩包
- 解压缩后得到DSH-Desktop解压包-0.3.7.exe
- 运行exe程序时，自动解压文件至当前目录
- 解压完成时，自动运行程序
- 第一次运行时，时间可能会久一点，请耐心等待

**说明：**

1.保持原生，仅增加一些实用功能
2. 只携带“插件市场”插件
3.可在不同的文件夹内解压，可以多开，理论上可以用另一个客户端修复前一个
4.设置-通用设置增加Enter（回车）功能，设置为换行时，自动改为Ctrl+Enter发送对话
5.设置-通用设置增加关闭按钮的功能，可设置最小化与直接关闭程序
6.设置-通用设置增加标题栏颜色，可设置跟随软件、跟随系统、自定义（取色盘）
7.设置-通用设置增加在系统盘常驻与开机自启动
8.设置-通用设置增加合并记录，合并记录说明见下方
9.设置-通用设置增加修复客户端，修复客户端说明见下方
10.设置-通用设置增加检查更新，可检测dsh官方版本，并更新

**合并记录说明：**

合并记录在第一次解压运行会提醒，超过1分钟不选择会以全新客户端进入，设置里也可找到合并记录并更新
合并记录包括会话存档、插件、设置、API。原因在于客户端崩溃，用新客户端时可以快速部署。
合并记录如果有多个文件夹的方式，会列出所有的地址，可以自由选择合并哪个文件
合并记录的会话、插件、设置、API也可以随意选择
注意：如果某些情况下客户端出错，用新客户端合并时，谨慎合并会话记录。偶有发生记录不全的现象。

**修复客户端说明：**

修复客户端目前的设定为，当客户端出现问题无法启动，具有自我修复的操作。出现问题后，可点击修复。也可在正常情况下通过设置里点击修复，用来检查问题。这项功能未得到充分测试，也许是无效的，也许只能当做一个状态查看器，不得而知。

